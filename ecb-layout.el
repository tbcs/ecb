...
(defun ecb-enable-temp-buffer-shrink-to-fit (arg)
  "Enables `temp-buffer-resize-mode' when a comile-window is used. When the
...
;;                                  t))
  ...
  (if ecb-layout-prevent-handle-ecb-window-selection
    ...
    (when (and ecb-minor-mode
               ...
               (or ;; TODO: Klaus Berndl <klaus.berndl@sdm.de>: rename this so
                   ...
                   (let ((edit-window-list (ecb-canonical-edit-windows-list))
                     ...
                     (unwind-protect
                         (progn
                           ...
                           (ecb-with-unsplittable-ecb-frame ad-do-it))
  ...
  (let ((ecb-win-width-before (window-width (frame-first-window ecb-frame)))
    ...
    (when (/= (window-width (frame-first-window ecb-frame))
      ...
      (save-selected-window
        ...
        (enlarge-window (+ curr-edit-win-width 0) t)))))
...
;;     `mouse-drag-mode-line' and `mouse-drag-vertical-line' (new after advices)??
            ...
            (if should-shrink
                (progn
                  ...
                  (when (not (ecb-reset-compile-window-specified-height-config))
                    ...
                    ;; destroys the window-layout (e.g. the topmost
                    ;; ecb-window disappears, when doing completion etc..) we
                    ;; have to ignore errors here.... it's not easy to find out
...
;; This function takes into account the value of of `temp-buffer-resize-mode' so
;; all the callers can profit: pop-to-buffer (==> switch-to-buffer-other-window
;; too), display-buffer (if interactive) and switch-to-buffer.
;;
...
;; LocalWords:  berndl Exp eval util speedbar defvar defun cl defs
